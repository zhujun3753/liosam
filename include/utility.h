...
public:
    ...
    ParamServer()
    {
        // bool param<T>(const std::__cxx11::string &param_name, T &param_val, const T &default_val) const
        // 从 config 中 params.yaml 文件读取参数设置
        // nh.param< 类型由数值决定,参考default值 >(key_name, value, default)
...
void imuRPY2rosRPY(sensor_msgs::Imu *thisImuMsg, T *rosRoll, T *rosPitch, T *rosYaw)
{
    ...
    // 把   quaternionMsg  形式的四元数转化为tf形式，得到orientation
